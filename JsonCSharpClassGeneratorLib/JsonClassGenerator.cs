...
namespace Xamasoft.JsonClassGenerator
{
    public class JsonClassGenerator : IJsonClassGeneratorConfig
    {
            ...
            if (writeToDisk)
            {
                ...
                else
                {
                    ...
                    foreach (var type in Types)
                    {
                        ...
                        var fileName = (UseNestedClasses && !type.IsRoot ? MainClass + "." : string.Empty) + type.AssignedName + CodeWriter.FileExtension;
                        if ((CodeWriter is ALTableCodeWriter || CodeWriter is ALPageCodeWriter || CodeWriter is ALApiPageCodeWriter))
                            fileName = (UseNestedClasses && !type.IsRoot ? MainClass + "." : SecondaryNamespace) + type.AssignedName + CodeWriter.FileExtension;
                        WriteClassesToFile(Path.Combine(folder, fileName), new[] { type });
                    }
                }
            }
        ...
        public static readonly string[] FileHeader = new[] {
            ...
            "Forked for AL language implementation",
            "https://businesscentraldotblog.wordpress.com/2021/01/08/my-json-to-al-converter/"
        };
    }
}
